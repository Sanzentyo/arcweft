...
fn environment_and_element_states_match_together_and_cache_key_tracks_environment() {
    let program = ViewStyleProgram::try_new(
        vec![sheet(
            ...
            vec![rule(
                1,
                vec![
                    ViewStylePredicate::ElementState(ViewElementState::FocusVisible),
                    ViewStylePredicate::Environment(ViewEnvironmentPredicate::ColorScheme(
                        ViewStyleComparison::Equal,
                        ColorScheme::Dark,
                    )),
                    ViewStylePredicate::Environment(ViewEnvironmentPredicate::ReduceMotion(true)),
                ],
                color(9, 9, 9),
            )],
        )],
    ...
    let dark = PresentationEnvironment::new(ColorScheme::Dark)
        ...
        .with_revision(EnvironmentRevision(1));
    let light = PresentationEnvironment::new(ColorScheme::Light)
        .with_reduce_motion(true)
        .with_revision(EnvironmentRevision(2));
...
fn nested_boundary_exposes_only_the_direct_root_or_an_explicit_exported_part() {
    ...
    let private = resolve(
        2,
        &ViewStyleNodeFacts::new(Some(ViewElementKind::Button))
            .with_parts(Some(ViewPartName::try_new("private.action").unwrap()), None),
