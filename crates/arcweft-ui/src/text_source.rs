...
pub enum UiTextSource {
    Plain(String),
    Localized(TextKey),
    RichTextDocument(UiRichTextHandle),
    DisplayFrame(UiRichTextHandle),
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct UiRichTextHandle {
    id: PublicId,
    revision: u64,
...
pub struct UiTextSourceTable {
    sources: BTreeMap<TextSourceId, UiTextSource>,
    next: u32,
}
...
impl UiTextSource {
    ...
    pub const fn rich_text_document(handle: UiRichTextHandle) -> Self {
        Self::RichTextDocument(handle)
    }

    pub const fn display_frame(handle: UiRichTextHandle) -> Self {
        Self::DisplayFrame(handle)
    }
}

impl UiRichTextHandle {
    ...
    #[must_use]
    pub const fn with_range(mut self, range: UiTextByteRange) -> Self {
...
impl UiTextSourceTable {
    pub fn insert(&mut self, source: UiTextSource) -> Result<TextSourceId, UiError> {
    ...
    pub fn get(&self, id: TextSourceId) -> Option<&UiTextSource> {
        self.sources.get(&id)
    }
