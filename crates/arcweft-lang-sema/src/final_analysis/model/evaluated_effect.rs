...
impl CheckedEvaluatedEffect {
    ...
    pub(crate) fn visit_types<E>(
        ...
        self.operation.visit_types(visitor)
    }
}
...
impl CheckedEffectField {
    pub(crate) fn visit_types<E>(
        ...
        self.operand.visit_types(visitor)
    }
}

impl CheckedDropFade {
    pub(crate) fn visit_types<E>(
        &self,
        visitor: &mut impl FnMut(&TypeKind) -> Result<(), E>,
    ) -> Result<(), E> {
        match self {
            Self::Constant(_) => Ok(()),
            Self::Operand(operand) => operand.operand().visit_types(visitor),
        }
    }
}

impl CheckedExplicitDropPolicy {
    pub(crate) fn visit_types<E>(
        &self,
        visitor: &mut impl FnMut(&TypeKind) -> Result<(), E>,
    ) -> Result<(), E> {
        match self {
            Self::Stop { fade } => fade.visit_types(visitor),
            Self::Cancel | Self::Finish | Self::Release | Self::Detach => Ok(()),
        }
    }
}

impl CheckedEvaluatedEffectOperation {
    pub(crate) fn visit_types<E>(
        &self,
        visitor: &mut impl FnMut(&TypeKind) -> Result<(), E>,
    ) -> Result<(), E> {
        match self {
            ...
            | Self::EmitEvent {
                ...
                message.visit_types(visitor)?;
                for field in fields {
                    field.visit_types(visitor)?;
                }
                Ok(())
            }
            Self::SignalWrite { target, value } | Self::MetricWrite { target, value } => {
                target.visit_types(visitor)?;
                value.visit_types(visitor)
            }
            Self::Panic { message } | Self::Fail { message } | Self::Bail { message } => {
                message.visit_types(visitor)
            }
            Self::Ensure { condition, message } => {
                condition.visit_types(visitor)?;
                message.visit_types(visitor)
            }
            Self::Drop { target, invocation } => {
                target.visit_types(visitor)?;
                if let CheckedDropInvocation::DropWithPolicy { source, policy } = invocation {
                    source.operand().visit_types(visitor)?;
                    policy.visit_types(visitor)?;
                }
                Ok(())
            }
        }
