...
fn removed_scaffold_declarations_do_not_reach_typechecked_hir() {
    for source in [
        ...
        let rejected = !parsed.errors().is_empty()
            || lower_to_hir(parsed.typed_tree()).is_err()
            || lower_to_hir(parsed.typed_tree()).is_ok_and(|hir| {
...
"#,
    ...
    let Item::Proof(trusted) = &tree.items()[1] else {
...
fn malformed_trust_metadata_and_separate_axiom_declarations_do_not_lower_as_proofs() {
    for source in [
        ...
        if let Ok(hir) = lower_to_hir(parsed.typed_tree()) {
            assert!(
                !hir.declarations()
                    .iter()
