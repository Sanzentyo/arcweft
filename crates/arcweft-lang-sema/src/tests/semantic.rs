...
#[test]
fn semantic_pass_accepts_justified_assume_and_known_axiom_use() {
    let report = semantic_report(
        r#"
trusted axiom @axiom.manifest_hashes {
    reason = "generated by signed build tooling"
}

proof @proof.axiom_justified {
    use @axiom.manifest_hashes
    assume no_lifetime_below(LineSummary, 'flow), axiom = @axiom.manifest_hashes
    check no_lifetime_below(LineSummary, 'flow)
}

flow @flow.proven proven {
    let summary = promote('flow, proof = @proof.axiom_justified)
}
"#,
        &TypeCheckEnv::new(),
    );
    ...
    assert!(report.obligations.iter().any(|obligation| {
        obligation.kind == SemanticObligationKind::LifetimePromotion
            && matches!(
                obligation.discharge,
                SemanticDischarge::FormalProof { ref id } if id == "proof.axiom_justified"
            )
    }));
}

#[test]
fn semantic_pass_reports_unknown_axiom_in_proof_body() {
    let report = semantic_report(
        r"
...
",
    ...
    assert!(report.obligations.iter().any(|obligation| {
        obligation.kind == SemanticObligationKind::ProofBody
            && obligation.discharge == SemanticDischarge::Missing
            && obligation.subject.as_deref() == Some("axiom.missing")
    }));
}
