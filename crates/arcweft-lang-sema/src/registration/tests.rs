...
const PACKAGE: &str = "registration-tests";

fn sample_manifest_for(owner: &str, asset: &str) -> CharacterManifest {
...
fn sample_manifest(asset: &str) -> CharacterManifest {
...
fn source_document(id: &str, source: impl Into<Arc<str>>) -> Arc<SourceDocument> {
    Arc::new(
...
fn root_project(profile: &str) -> (Arc<SourceDocument>, HirProject, ProjectSymbolWorldId) {
    root_project_source(profile, "fn main() -> Unit { () }\n")
}

fn root_project_source(
    profile: &str,
    source: &str,
...
fn declaration_span(manifest: &SourceBackedCharacterManifest) -> arcweft_source::SourceSpan {
    manifest
        .source_map()
...
fn one_character_facts_with_documents(
    root: &Arc<SourceDocument>,
...
fn register(
    ...
    base: TypeCheckEnv,
    previous: Option<&super::RegisteredTypeCheckEnv>,
) -> Result<RegisteredSemanticWorld, super::CharacterRegistrationReport> {
