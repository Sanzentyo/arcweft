//! Retained-View lowering for typed Arcweft character compositions.
//!
...
//! `ViewLayerOutput` used by ordinary Arcweft View components.
...
pub enum CharacterViewCompatibility {
...
pub struct CharacterView {
    ...
    image_sources: ViewImageSourceTable,
    root: NodeId,
    layers: Vec<CharacterViewLayer>,
}
...
pub enum CharacterViewError {
    ...
    #[error(
        "character layer `{part}.{variant}` uses unsupported retained-View blend mode {blend:?}"
    )]
    UnsupportedBlend {
        part: String,
        variant: String,
    ...
    #[error(
        "character layer `{part}.{variant}` uses clipping, which retained View cannot reproduce"
    )]
    UnsupportedClipping { part: String, variant: String },
    #[error("character coordinate or dimension cannot be represented by retained View")]
    CoordinateOverflow,
    ...
    pub fn from_png_files(
        ...
        for (path, bytes) in files {
            let image = decode_image_bytes(ImageFormat::Png, &bytes, ImageDecodeOptions::default())
                .map_err(|source| CharacterViewError::ImageDecode {
...
impl CharacterView {
    /// Lowers a resolved character render spec to retained View image nodes.
    pub fn build(
        render: &CharacterRenderSpec,
        images: &CharacterImageSet,
        compatibility: CharacterViewCompatibility,
    ) -> Result<Self, CharacterViewError> {
        ...
        for layer in render.layers() {
            ...
            let image = images.get(layer.asset_path()).cloned().ok_or_else(|| {
                CharacterViewError::MissingImage(layer.asset_path().as_str().to_owned())
            })?;
            ...
            layers.push(CharacterViewLayer {
                ...
                LayoutSize::new(
                    LayoutLength::px(
                        i32::try_from(canvas.width())
                            .map_err(|_| CharacterViewError::CoordinateOverflow)?,
                    ),
                    LayoutLength::px(
        ...
        let output =
            ViewLayerOutput::from_fragment(&fragment, &layouts, ViewSemanticFragment::default())?;
    ...
    /// Display/semantic output consumed by the View frame commit path.
    pub const fn output(&self) -> &ViewLayerOutput {
        &self.output
    }

    /// Decoded image sources referenced by `FragmentKind::Image` nodes.
    pub const fn image_sources(&self) -> &ViewImageSourceTable {
        &self.image_sources
    }
    ...
    pub fn layers(&self) -> &[CharacterViewLayer] {
        &self.layers
    }
    ...
    ) -> Result<ViewResolvedImageFrame<'_>, CharacterViewError> {
        let layer = self
            .layers
            .get(index)
            .ok_or(CharacterViewError::LayerOutOfRange(index))?;
        let layout = self.layouts.require(layer.node)?;
        self.image_sources
            .resolve_frame(layer.image, layout, visual_time_millis)
            .map_err(CharacterViewError::from)
    }
}

impl CharacterViewLayer {
...
fn check_compatibility(
    ...
    if compatibility == CharacterViewCompatibility::PreserveMetadata {
        return Ok(());
    }
    if !layer.blend().is_retained_view_supported() {
        return Err(CharacterViewError::UnsupportedBlend {
    ...
    if layer.clipping() {
        return Err(CharacterViewError::UnsupportedClipping {
            part: layer.part().to_string(),
            variant: layer.variant().to_string(),
        });
    }
...
fn check_dimensions(
    ...
    if actual.width() == expected.width() && actual.height() == expected.height() {
        ...
        Err(CharacterViewError::ImageDimensionMismatch {
...
fn rect_layout(rect: CharacterRect) -> Result<LayoutBox, CharacterViewError> {
    let width = i32::try_from(rect.width()).map_err(|_| CharacterViewError::CoordinateOverflow)?;
    let height =
        i32::try_from(rect.height()).map_err(|_| CharacterViewError::CoordinateOverflow)?;
    Ok(LayoutBox::new(
...
mod tests {
    ...
    fn builds_retained_image_stack_from_render_spec() {
        ...
        let view = CharacterView::build(&render, &images, CharacterViewCompatibility::Strict)
            .expect("view");
    ...
    fn strict_mode_rejects_unimplemented_blend_behavior() {
        ...
        assert!(matches!(
            CharacterView::build(&render, &images, CharacterViewCompatibility::Strict),
            Err(CharacterViewError::UnsupportedBlend { .. })
        ));
    }
}
