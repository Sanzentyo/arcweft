            ...
            match instruction {
                ViewProgramInstruction::OpenElement {
                    ...
                    let target = match binding {
                        ...
                        RuntimeStyleBinding::None => target
                            .clone()
                            .unwrap_or_else(|| runtime_element_target(*element, part.as_deref())),
                    };
...
fn next_text_control_binding(
    ...
    cursor: &mut usize,
    kind: Option<ViewInputKind>,
    target: Option<&str>,
) -> RuntimeStyleBinding {
    if let Some(target) = target
        && let Some(index) = controls
            .iter()
            .position(|control| id_or_tail_matches(target, &control.public_id))
    {
...
fn next_action_button_binding(
    ...
    if let Some(target) = target
        && let Some(index) = buttons
            .iter()
            .position(|button| id_or_tail_matches(target, &button.public_id))
    {
        *cursor = (*cursor).max(index.saturating_add(1));
        return RuntimeStyleBinding::ActionButton(index);
    }
