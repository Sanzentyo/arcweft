...
fn ident_pattern(pattern: &Pattern, expected: &str) -> bool {
...
fn parses_expression_shapes_needed_by_hir_lowering() {
    ...
    let method =
        super::parse_expr("choices.filter(_.enabled).map(_.label)").expect("method chain parses");
    ...
    let partial = super::parse_expr("_.score >= ^").expect("partial comparison expression parses");
    ...
    let record_literal =
        super::parse_expr("{ player_name = state.player_name }").expect("record literal parses");
    ...
    let membership = super::parse_expr("progress in 0.0..=1.0").expect("range membership parses");
    assert!(matches!(
        membership,
        Expr::Binary {
            op: BinaryOp::In,
