...
/// Maximum number of argument entries retained for one dialogue point action.
pub const MAX_DIALOGUE_ACTION_ARGUMENTS: usize = 32;
/// Maximum UTF-8 byte length of one `RichText` argument key.
pub const MAX_DIALOGUE_ACTION_KEY_BYTES: usize = 64;
/// Maximum encoded and decoded UTF-8 byte length of one `RichText` value.
pub const MAX_DIALOGUE_ACTION_VALUE_BYTES: usize = 4_096;
/// Maximum UTF-8 byte length of one dialogue point-action head.
pub const MAX_DIALOGUE_ACTION_HEAD_BYTES: usize = 16_384;
/// Maximum number of point-action nodes retained in one dialogue content.
pub const MAX_DIALOGUE_POINT_ACTIONS: usize = 4_096;
/// Maximum total `RichText` argument entries retained in one dialogue content.
pub const MAX_DIALOGUE_ACTION_ARGUMENTS_TOTAL: usize = 32_768;
...
/// Quote-aware closing boundary of one authored dialogue point action.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct DialogueBracketBoundary {
    close: usize,
    unterminated_quote_start: Option<usize>,
}

impl DialogueBracketBoundary {
...
/// Finds a point-action boundary without reading past an already accepted dialogue
/// content boundary.
///
...
pub(crate) fn find_dialogue_bracket_boundary(
    source: &str,
    open: usize,
    end: usize,
) -> Option<DialogueBracketBoundary> {
    ...
    for (relative, ch) in source.get(start..end)?.char_indices() {
        ...
        if matches!(ch, '"' | '\'') {
            ...
            return Some(DialogueBracketBoundary {
                close: index,
                unterminated_quote_start: None,
            });
        }
    }
    quoted_close.map(|close| DialogueBracketBoundary {
        close,
...
pub(crate) struct ScannedDialogueActionArguments {
    entries: Vec<ScannedDialogueActionArgument>,
    diagnostics: Vec<DialogueTextDiagnostic>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) enum ScannedDialogueActionArgument {
    Positional {
...
pub(crate) struct ScannedDialogueActionArgumentParts {
    name: Option<TextRange>,
    equals: Option<TextRange>,
    value: Option<TextRange>,
}

impl ScannedDialogueActionArgumentParts {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct ScannedDialogueActionArgumentValue {
    decoded: String,
...
impl ScannedDialogueActionArguments {
    pub(crate) fn entries(&self) -> &[ScannedDialogueActionArgument] {
        &self.entries
    }
...
impl ScannedDialogueActionArgument {
...
impl ScannedDialogueActionArgumentValue {
...
pub(crate) fn scan_dialogue_action_arguments(
    source: &str,
    base: usize,
    content_arguments_remaining: usize,
) -> ScannedDialogueActionArguments {
    let mut parsed = ScannedDialogueActionArguments::default();
    let mut cursor = 0;
    while cursor < source.len() {
        cursor += source[cursor..]
            .chars()
            .take_while(|ch| is_dialogue_action_whitespace(*ch))
            .map(char::len_utf8)
            .sum::<usize>();
        if cursor >= source.len() {
            break;
        }
        if parsed.entries.len() >= MAX_DIALOGUE_ACTION_ARGUMENTS {
            parsed.diagnostics.push(DialogueTextDiagnostic::with_code(
                ...
                format!(
                    "dialogue point action has more than {MAX_DIALOGUE_ACTION_ARGUMENTS} arguments"
                ),
...
fn find_argument_boundary(source: &str, start: usize) -> ArgumentBoundary {
    ...
    for (relative, ch) in source[start..].char_indices() {
        ...
        if matches!(ch, '"' | '\'') {
            quote = Some(ch);
            quote_start = Some(index);
        } else if is_dialogue_action_whitespace(ch) {
            end = index;
            break;
        }
    }
...
fn scan_dialogue_action_argument(
    source: &str,
    start: usize,
    range: TextRange,
    diagnostics: &mut Vec<DialogueTextDiagnostic>,
) -> ScannedDialogueActionArgument {
    let Some(equal) = unquoted_assignment(source) else {
        return match scan_dialogue_action_argument_value(source, start) {
            Ok(value) => ScannedDialogueActionArgument::Positional { value, range },
            Err(failure) => reject_dialogue_action_argument(
                failure.issue,
                failure.range,
                range,
                ScannedDialogueActionArgumentParts::positional(range),
                diagnostics,
            ),
        };
    };
    ...
    if key.is_empty() {
        ...
        return reject_dialogue_action_argument(
            issue,
            key_range,
            range,
            ScannedDialogueActionArgumentParts::named(
                key_range,
                equals_range,
    ...
    if key.len() > MAX_DIALOGUE_ACTION_KEY_BYTES {
        let limit = utf8_boundary_at_or_before(key, MAX_DIALOGUE_ACTION_KEY_BYTES);
    ...
    let value = if authored_value.is_empty() {
        ...
        diagnostics.push(DialogueTextDiagnostic::with_code(
            DialogueTextDiagnosticCode::DialogueActionArgumentMissingValue,
            missing_range,
            format!("dialogue RichText argument `{key}` is missing a value"),
            "insert an authored value after `=`",
        ));
        return ScannedDialogueActionArgument::Invalid {
            range,
            issue: RichTextArgumentIssue::MissingValue,
            issue_range: missing_range,
            parts: ScannedDialogueActionArgumentParts::named(key_range, equals_range, None),
        };
    } else {
        match scan_dialogue_action_argument_value(authored_value, value_absolute_start) {
            Ok(value) => value,
            Err(failure) => {
                return reject_dialogue_action_argument(
                    failure.issue,
...
fn reject_dialogue_action_argument(
    issue: RichTextArgumentIssue,
    issue_range: TextRange,
    range: TextRange,
    parts: ScannedDialogueActionArgumentParts,
...
fn scanned_argument_parts(source: &str, start: usize) -> ScannedDialogueActionArgumentParts {
    let whole = TextRange::new(start, start + source.len());
    let Some(equal) = unquoted_assignment(source) else {
        return ScannedDialogueActionArgumentParts::positional(whole);
    };
    let name = TextRange::new(start, start + equal);
    let equals = TextRange::new(start + equal, start + equal + '='.len_utf8());
    ScannedDialogueActionArgumentParts::named(name, equals, authored_value_range(whole, equals))
}
...
pub(super) fn scan_dialogue_action_argument_value(
    source: &str,
    start: usize,
) -> Result<ScannedDialogueActionArgumentValue, RichTextArgumentFailure> {
    let token_range = TextRange::new(start, start + source.len());
    if source.len() > MAX_DIALOGUE_ACTION_VALUE_BYTES {
        let limit = utf8_boundary_at_or_before(source, MAX_DIALOGUE_ACTION_VALUE_BYTES);
    ...
    let value = decode_dialogue_action_argument_value(content, content_start)?;
    if value.len() > MAX_DIALOGUE_ACTION_VALUE_BYTES {
        return Err(RichTextArgumentFailure::new(
            RichTextArgumentIssue::ValueTooLong,
            TextRange::new(content_start, content_start + content.len()),
        ));
    }
    Ok(ScannedDialogueActionArgumentValue {
        decoded: value,
        token_range,
...
pub(crate) fn scan_dialogue_action_argument_value_if_valid(
    source: &str,
    start: usize,
) -> Option<ScannedDialogueActionArgumentValue> {
    scan_dialogue_action_argument_value(source, start).ok()
}
...
fn decode_dialogue_action_argument_value(
    source: &str,
...
fn argument_issue_diagnostic(
    ...
    match issue {
        RichTextArgumentIssue::EmptyKey => DialogueTextDiagnostic::with_code(
            DialogueTextDiagnosticCode::DialogueActionArgumentEmptyKey,
            range,
            "dialogue RichText argument key is empty",
            "insert a key matching `[a-z][a-z0-9_]*` before `=`",
        ),
        RichTextArgumentIssue::InvalidKey => DialogueTextDiagnostic::with_code(
            DialogueTextDiagnosticCode::DialogueActionArgumentInvalidKey,
            range,
            "dialogue RichText argument key is not canonical",
        ...
        RichTextArgumentIssue::InvalidEscape | RichTextArgumentIssue::DecoderFailure => {
            DialogueTextDiagnostic::with_code(
                DialogueTextDiagnosticCode::DialogueActionArgumentInvalidEscape,
                range,
                "dialogue RichText argument contains an unsupported escape",
                "use one of `\\\\`, `\\\"`, `\\'`, `\\n`, `\\r`, `\\t`, `\\ `, `\\=`, `\\[`, or `\\]`",
            )
        }
        RichTextArgumentIssue::UnterminatedQuote => DialogueTextDiagnostic::with_code(
            ...
            "close the quoted action argument before `]`",
        ),
        RichTextArgumentIssue::KeyTooLong => DialogueTextDiagnostic::with_code(
            ...
            range,
            format!("dialogue RichText argument key exceeds {MAX_DIALOGUE_ACTION_KEY_BYTES} bytes"),
            "shorten the argument key",
        ),
        RichTextArgumentIssue::ValueTooLong => DialogueTextDiagnostic::with_code(
            ...
            range,
            format!(
                "dialogue RichText argument value exceeds {MAX_DIALOGUE_ACTION_VALUE_BYTES} bytes"
            ),
            "shorten the argument value",
        ),
        RichTextArgumentIssue::MissingValue => DialogueTextDiagnostic::with_code(
            DialogueTextDiagnosticCode::DialogueActionArgumentMissingValue,
            range,
            "dialogue RichText argument is missing a value",
            "insert an authored value after `=`",
        ),
    }
}

pub(crate) fn is_dialogue_action_whitespace(ch: char) -> bool {
    matches!(
        ch,
        '\u{0009}'..='\u{000D}'
            | '\u{0020}'
...
pub(crate) fn trim_dialogue_action_whitespace(source: &str) -> &str {
    trim_dialogue_action_whitespace_start(source).trim_end_matches(is_dialogue_action_whitespace)
}

fn trim_dialogue_action_whitespace_start(source: &str) -> &str {
    source.trim_start_matches(is_dialogue_action_whitespace)
}
...
mod scanner_tests {
    use super::{
        RichTextArgumentIssue, ScannedDialogueActionArgument, scan_dialogue_action_arguments,
    };
    use crate::ast::common::TextRange;
    ...
    fn scanner_retains_decoded_value_and_exact_authored_ranges() {
        ...
        let base = 17;
        let scanned = scan_dialogue_action_arguments(source, base, 32);
    ...
    fn scanner_classifies_missing_named_value_as_invalid_without_losing_parts() {
        ...
        let ScannedDialogueActionArgument::Invalid {
            issue,
            issue_range,
            parts,
            ..
