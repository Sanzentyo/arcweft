...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    fs::create_dir_all("target/css-style-parity")?;

    let mut samples = Vec::new();
    run_step(
        &mut samples,
        "bundle css-style-parity",
        CommandSpec::new("cargo").args([
            ...
            "--output",
            "web/local/css-style-parity.awfb",
        ]),
    )?;
    run_step(
        &mut samples,
    ...
    println!(
        "wrote css-style parity startup profile {}",
        args.output.display()
    );
    Ok(())
}
...
impl Args {
    fn parse(args: impl IntoIterator<Item = String>) -> Result<Self, Box<dyn Error>> {
        let mut output = PathBuf::from("target/css-style-parity/startup-profile.json");
        ...
        while let Some(arg) = args.next() {
            match arg.as_str() {
                ...
                "--help" | "-h" => {
                    println!(
                        "Usage: cargo +nightly -Zscript tools/profile-css-style-parity-startup.rs [--output <path>]"
                    );
                    std::process::exit(0);
                }
