...
- what frame, viewport, layers, views, objects, and rich-text child ranges exist
- which full-frame, view, layer, object, and rich-text child captures are available
...
| `renderer` | `native` |
| `scope` | `viewport`, `view { id }`, `layer { id }`, `object { id }` |
...
- view color crop or isolated view image
- view object-id and mask image
...
View-scoped image metadata carries a `view` reference in addition to
`scope.view.id`. The reference preserves the view id, optional parent id, object
count, member object refs, bbox, and view capture refs so clients can inspect
the captured view without deriving membership from URI or object name
conventions.
...
`scope = view { id }` captures the union of visible objects belonging to an
observed view boundary. Current native observation conservatively groups by
`parent_id` when available and otherwise by entity/id until authored view
identity is exposed by frame lowering. The view descriptor lists its available
`capture_refs`.
...
arcweft://session/{session}/frame/{frame}/view.{id}.png
arcweft://session/{session}/frame/{frame}/view.{id}.rgba
arcweft://session/{session}/frame/{frame}/view.{id}.object-id.png
arcweft://session/{session}/frame/{frame}/view.{id}.object-id.rgba
arcweft://session/{session}/frame/{frame}/view.{id}.mask.png
arcweft://session/{session}/frame/{frame}/view.{id}.mask.rgba
