...
metadata file for the matching crate. Dedicated profile commands such as
`arcw serve --manifest arcw.toml --profile server.dev` preserve this profile
metadata when the adapter is selected by the profile; an explicit command-line
`--adapter` remains an adapter override.
