...
resolved before parsing and typechecking. The selected standard adapter and
exact generated external-module metadata contribute typed symbols, methods,
host calls, effect capabilities, and Rust ABI facts to semantic verification as
data rather than hard-coded language behavior.
...
package/version/module/family/ABI identity before merging public exports into
the selected semantic adapter view. Direct source checks intentionally receive
no project external-module bindings. Dedicated profile commands such as
`arcw serve --manifest arcw.toml --profile server.dev` preserve the selected
metadata; an explicit command-line `--adapter` remains a standard-adapter
override and does not create source-authored external declarations.
...
`resource.mime_type`, and `resource.hash`, plus body fields such as
...
[--remote-command <cmd>] [--remote-arg <arg>]...
...
embedded/skipped chunk ids. `--provider remote` uses an explicit command
adapter: Arcweft writes a JSON request to the command's stdin and reads a JSON
response from stdout. The request has `schema_version`, `model`, and
privacy-filtered `inputs`; the response must contain
`{"embeddings":[{"chunk_id":"...","values":[...]}]}` for every supplied input.
Arcweft validates chunk ids, vector dimensions, finite values, and non-zero
norms before storing normalized vectors. If `--provider remote` is selected
without `--remote-command`, the command records a searchable
`AGENT_DEBUG_EMBEDDING_PROVIDER_UNAVAILABLE` diagnostic instead of generating
placeholder vectors. Provider SDKs and HTTP clients belong in that external
bridge process, not in Sans I/O debug/RAG crates.
...
Declaration linting uses the same diagnostic names as LSP. Source-level
...
forms. The parser accepts them, while check and LSP report
`style::redundant_decl_identity` for hand-written source. Generated source can opt in with `#[generated]` or
