...
- Sema now accepts named missing-input partial application for top-level
  ...
  emits a `RuntimeExpr::Function` that calls the pure helper with provided
  named inputs in helper input order.
...
- Extend partial call abstraction lowering beyond known positional signatures.
  ...
  helper input names, including top-level pure-helper named missing inputs;
  spread arguments remain open.
...
- `let add_to_one = add(right = 1i64)`. Covered by
  `pure_function_named_missing_input_typechecks_as_partial_application`,
  `strict_runtime_lowers_named_missing_pure_helper_input_to_function`, and
  `runtime_plan_lowers_named_missing_pure_helper_input`.
