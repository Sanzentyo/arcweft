...
- Sema now reports data-last fallback candidates that use named or spread
  arguments with a structured `UnsupportedDataLastMethodFallback` diagnostic
  instead of falling through to `unknown method`. Executable data-last fallback
...
4. Method-chain fallback uses typed callable evidence for positional data-last
   ...
   candidates now produce structured unsupported diagnostics. Real env/inherent/
   trait methods remain authoritative, and viable fallback candidates hidden by
   those methods now surface as shadowing warnings. Executable named/spread
   fallback lowering and curried call-group runtime metadata remain open.
...
- Named/spread data-last fallback candidates report structured unsupported
  diagnostics instead of generic unknown-method errors. Covered by
  `method_chain_reports_named_data_last_fallback_as_unsupported` and
