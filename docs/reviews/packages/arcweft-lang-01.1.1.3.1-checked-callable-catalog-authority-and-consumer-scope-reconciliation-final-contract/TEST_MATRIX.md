...
All rows are mandatory unless explicitly marked non-applicable. The retained parent matrix is interpreted through this correction: any reference to checked signature/source/access metadata means delegation to the exact retained `Arc<CallableRecord>`, and any fixed external/standard row means `CheckedCallableEffects::RecordFixed`. Tests use typed APIs, behavior, diagnostics, codecs, compile-fail visibility, pointer identity, or structured dependency evidence. Repository source-text searching is prohibited.

## Retained Lang-01.1.1.3 matrix
...
| T010S | traits | positive | programmatic standard bodyless implementation | RecordFixed delegates to the exact accepted record row; no body inference, row copy, or fake source | sema |
...
| Z006 | behavior removal | positive | trait method effect lookup | checked catalog query delegates metadata to the record and returns authored/inferred row; no hard-coded empty | sema |
...
## Lang-01.1.1.3.1 catalog-authority correction rows

| ID | Layer | Polarity | Case | Required result | Focused gate |
|---|---|---|---|---|---|
| C001 | accepted catalog | positive | project function checked fact | `facts.record()` is `Arc::ptr_eq` to `RegisteredCallableCatalog::project_record` | sema catalog |
| C002 | accepted catalog | positive | project View checked fact | exact record Arc retained; signature/source/docs/access/provider read through record | sema catalog |
| C003 | accepted catalog | positive | trait requirement checked fact | exact by-declaration record Arc retained; no method signature/source copy | sema traits |
| C004 | accepted catalog | positive | trait implementation/inherent records | each structural key maps to one exact record Arc; method storage retains ID/body only | sema traits |
| C005 | accepted catalog | positive | registered environment callable | `EnvironmentCallableId` checked fact retains exact environment catalog record Arc | sema catalog |
| C006 | accepted catalog | positive | registered standard method | standard checked fact retains exact accepted record Arc and version | sema standard |
| C007 | detached | positive | detached callable record | record Arc constructed once and exists only in pending/final checked fact; no public detached registry | sema detached |
| C008 | fixed row | positive | environment/standard fixed effect | `RecordFixed` has no payload and `exposed_row()` returns record schema row by reference | sema effects |
| C009 | source row | positive | project body contract | accepted record effect schema is ID-only; checked fact alone owns contract/inferred row | sema effects |
| C010 | divergence | negative | attempt equal-but-distinct record Arc | builder rejects `ForeignRecordArc`; value equality cannot substitute for pointer authority | sema transaction |
| C011 | divergence | compile-fail | construct `CheckedCallableFacts` with signature/source/access fields | fields/constructor unavailable | trybuild |
| C012 | divergence | compile-fail | construct fixed checked row payload | `ExternalOrStandard { exposed }` unavailable; only payload-free `RecordFixed` | trybuild |
| C013 | accepted API | compile-fail | call public `CallableRecord::try_new` | constructor is not public | trybuild |
| C014 | effect schema | compile-fail | initialize `CallableEffectSchema::Project { declared, .. }` | `declared` field unavailable | trybuild |
| C015 | catalog generation | negative | foreign registered catalog digest | no checked fact/report/index relation/Agent/persistent output | sema+compiler |
| C016 | catalog generation | negative | stale project world/revision with same declaration spelling | typed stale error; no raw-HIR/name fallback | sema+LSP |
| C017 | catalog generation | negative | stale standard catalog version | standard fact unavailable; whole publication fails closed | sema+compiler |
| C018 | catalog generation | negative | foreign detached source identity | no detached fact/source index/closure row | sema |
| C019 | rollback | positive | rollback after shell, candidate/source index, edge, conformance, closure row | all post-checkpoint checked state absent; accepted catalog unchanged | sema transaction |
| C020 | publication | positive | successful freeze | exactly one immutable `Arc<CheckedCallableCatalog>` published | sema API |
| C021 | Arc sharing | positive | TypeCheckReport -> ProjectSemanticIndex | `Arc::ptr_eq(report.checked_callables(), index.checked_callables())` | sema project index |
| C022 | Arc sharing | positive | CompiledProject/LSP accepted profile | same checked Arc and same registered catalog Arc/generation validated before publish | compiler+LSP |
| C023 | project symbol | positive | Function/View/method symbol | map keyed by structural `CallableDeclarationKey`; value stores checked ID/kind/interface digest only | project index |
| C024 | project symbol | compile-fail | read symbol signature/source/effect fields | fields are unavailable | trybuild |
| C025 | project lookup | compile-fail | call `project_callable(QualifiedName)` | name-based lookup is unavailable | trybuild |
| C026 | graph identity | positive | direct/bound/environment call edge | `ProjectGraphSymbolRef::Callable(CheckedCallableId)` from typed call fact | project index |
| C027 | graph identity | compile-fail | construct callable graph ref from `QualifiedName` | old variant unavailable | trybuild |
| C028 | collision | positive | same display name across Function/View/trait requirement/impl/inherent/environment | all structural and checked IDs, records, relations, Agent IDs remain distinct | sema+Agent |
| C029 | LSP declaration | positive | source cursor to checked record | exact source index -> checked ID -> retained record; schema/docs/row/source parity | LSP |
| C030 | LSP stale | negative | source version valid but checked catalog generation stale | response discarded; no spelling/raw-HIR fallback | LSP |
| C031 | Agent identity | positive | project callable graph symbol | ID is `project:callable:v1:<structural digest>`; name display-only | compiler Agent |
| C032 | Agent identity | positive | environment callable graph symbol | ID derives from `EnvironmentCallableId` digest; catalog generation validated | compiler Agent |
| C033 | Agent failure | negative | missing/foreign checked graph ref | no partial graph or function payload | compiler Agent |
| C034 | Agent authority | positive | function/signature/effect payload | output derives on demand from retained record/fact; index has no copied authoritative row | compiler Agent |
| C035 | persistent identity | positive | project callable interface row | exact structural declaration object/digest plus record signature and interface digests | compiler persistent |
| C036 | persistent identity | negative | inspect serialized object for checked context/row/source | no `CheckedCallableId`, row, source span, pointer, or local index is serialized | codec structured test |
| C037 | persistent source | negative | missing checked record during interface build | typed error and no object; no HIR signature reconstruction | compiler persistent |
| C038 | persistent stale | negative | catalog/standard/interface digest input differs | cache miss/rebuild; never accepts stale callable row | project persistent |
| C039 | persistent API | compile-fail | invoke old HIR-only callable interface builder | old callable-authority path unavailable | trybuild |
| C040 | runtime projection | positive | checked digest v2 | catalog/environment context bytes produce exact golden digest and `RuntimeCallableId` | sema+core |
| C041 | environment identity | positive | same lookup spelling from two environment owners/publications | distinct `EnvironmentCallableId`, checked ID, record, and durable Agent ID | sema+Agent |
| C042 | access owner | positive | alias/reexport and declaration visibility | record owns declaration access; binding owns edge visibility; no checked-symbol access copy | HIR+sema |
| C043 | provider/publication owner | positive | Rust/environment callable | provider/Rust/publication data observed through exact record by report/index/LSP | sema+LSP |
| C044 | metadata divergence | mutation/visibility | no public setter/update/struct literal for record metadata | one immutable record cannot diverge across consumers | API test |
| C045 | report authority | compile-fail | declaration effect lookup through old public `EffectAnalysisReport`/execution vector | old separate authority unavailable | trybuild |
| C046 | trait storage | compile-fail | read copied `TraitMethodRequirement.signature` or implementation return/row | copied fields unavailable; use checked record | trybuild |
| C047 | index reconstruction | compile-fail | call `ProjectSemanticIndex::typecheck_env()` | reconstructed type-check environment unavailable | trybuild |
| C048 | exact generation | positive | report/index/compiler/LSP/Agent/persistent generated from one build | all expose matching catalog generation/interface root | integration |
| C049 | fail closed | negative | any stale/foreign/rollback ID | zero checked record, row, relation, Agent payload, runtime method, or persistent fallback | integration |
| C050 | structured audit | positive | authority inventory | one accepted record owner, one checked fact owner, no forbidden dependency/fallback | structure audit |

## Correction-specific invariant tests

The following assertions are required in addition to row tests:

```rust
assert!(Arc::ptr_eq(
    checked.registered_catalog().unwrap(),
    world.environment().callable_catalog_arc(),
));

assert!(Arc::ptr_eq(
    report.checked_callables(),
    project_index.checked_callables(),
));

assert!(Arc::ptr_eq(
    facts.record(),
    registered.record(facts.record().id()).unwrap(),
));
```

The implementation provides the exact owning-module accessors named by `FINAL_CONTRACT.md`; the pointer relationships and failure behavior above are normative.
...
Tier 2 is mandatory because the switch changes multi-crate public semantic identity and reaches project/LSP/Agent/persistent/runtime paths. If the repository's exact Tier 2 recipe name has changed, use the current AGENTS/justfile recipe covering those same surfaces; do not omit the tier.

## Removal evidence rule

Removal evidence must be compile-fail visibility, typed behavior, typed diagnostics, canonical codec output, pointer identity, transaction outcome, Cargo metadata, or the repository structure audit. A test or review script must not search repository source text for forbidden symbol/path spellings and call that semantic evidence.
