...
The enclosing payload supplies the exact function/instruction, function/block, effect, plan, signature, pattern, or other table ID shown in `AWBC_SITE_CANONICAL_TAGS.csv`. For audio, the effect selects the command and signature before Arg/Const resolution. Slot/opcode agreement and actual referenced owner fields are checked before type/root correlation.
...
Only `RuntimePlanTypeKind::Checked` rows may enter the equality transcript; an Operational row named by an origin is rejected. One resolved checked plan row is:
