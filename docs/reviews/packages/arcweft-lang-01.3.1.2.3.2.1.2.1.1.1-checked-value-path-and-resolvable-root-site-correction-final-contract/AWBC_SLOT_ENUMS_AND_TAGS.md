...
`AwbcEffectPlan.audio` is an indirect reference to one `AwbcAudioCommandId`; its typed fields are represented only by the separate `AwbcTypedSite::AudioCommand` coordinate and `AWBC_AUDIO_TYPED_SLOTS.csv`, never by an unshaped `AudioValue(slot)`.
