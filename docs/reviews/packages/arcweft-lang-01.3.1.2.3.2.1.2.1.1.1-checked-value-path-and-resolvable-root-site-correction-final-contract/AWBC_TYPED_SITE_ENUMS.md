...
Owner: `crates/arcweft-core/src/awbc/typed_sites.rs`, re-exported from `arcweft_core::awbc`. Raw DTO fields are private; lowering uses crate-visible checked constructors and deserialization uses private wire DTOs followed by the same constructors.
...
pub enum AwbcTypedSite {
    ...
    AudioCommand { command: AwbcAudioCommandId, slot: AwbcAudioCommandTypedSlot },
...
impl AwbcTypedOrigin {
    pub(crate) fn try_new(
        plan_site: RuntimePlanTypedSite,
        awbc_site: AwbcTypedSite,
    ) -> Result<Self, AwbcTypedOriginError>;
...
`AwbcTypedOrigin` deliberately has no root ID, semantic ID, checked type, generation ID, or dense type ID. Those values are independently resolved from the actual plan owner and actual AWBC owner after each raw artifact has been admitted.
