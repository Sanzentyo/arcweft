...
- `docs/implementation/function-stack-source-function-type-effect-row-syntax-2026-07-09.md`
...
- `docs/implementation/function-stack-effect-row-raw-lsp-hover-2026-07-09.md`
...
- A narrow non-helper source-local top-level `fn` family now materializes as
  ...
  `RuntimeExpr::Apply`. Pure pipe expressions inside accepted source-function
  bodies are accepted when they lower through local function values, pure
  helpers, or accepted source-function candidates, including `^` substitution
  paths whose substituted expression stays in the accepted subset. Named
  missing-input partial calls synthesize wrapper
  functions that
...
- `EffectRowReport` now resolves into a typed `ClosedEffectRowReport`
  boundary, and Agent verified-effects manifest lowering consumes that closed
  boundary instead of reading graph summaries or resolving row tails directly.
...
- LSP callable declaration hover consumes the same closed boundary for current
  inferred/upper-bound/forbidden row display.
- LSP callable and closure effect-row hover now consume the raw owned row
  report and use `EffectRow::display_label()`, so future open rows can render
  without requiring closed projection first.
- `EffectAnalysisReport` now also owns a typed `EffectTraceReport` containing
  deterministic origin witnesses for inferred effects, so future row-origin
  consumers do not need to depend on effect diagnostics being emitted.
- Semantic `Function` types now carry `EffectRow` values. Known
  environment/project function values expose closed registered rows, partial
  function values preserve those rows, source function type syntax accepts
  closed row suffixes such as `A -> B effects { fs.read }`, and source labels
  render closed non-empty rows.
- Closed source function type rows used as closure expected types now become
  the synthetic closure callable's upper bound. This makes
  `let f: A -> B effects { e } = |x| ...` accept body effects covered by `e`,
  while `effects { }` rejects an effectful closure body with
  `UpperBoundExceeded`.
...
- Source-local wrapper functions that exact-call unaccepted source-local
  functions stay outside the runtime function-value family; checked
  runtime-plan lowering rejects their missing-input partial construction,
  data-last partial construction, and bare value references with the existing
  unsupported callable families.
...
| Non-helper/effectful/suspending callable allocation | Callable families are inventoried. The first non-helper source-local `fn` expansion is implemented for expression bodies with no host/effect/suspension syntax, including multiple curried `ParamGroup`s, returned simple closure literals, direct calls to function-typed parameters, local callback aliases/partials, destructuring closure literals in function-valued local bindings, exact calls to already-lowered pure helpers, pure value control expressions, and fixed-point exact calls to already-accepted source-local candidates. Bare task/dialogue/stream function values have focused structured rejection coverage. Data-last task/dialogue/stream partials have focused structured rejection coverage. Source-local wrappers that exact-call unaccepted source-local functions have focused structured rejection coverage for missing-input partial, data-last partial, and bare-value surfaces. Value-position environment, inherent, and trait/impl method references have focused structured rejection coverage. Effectful/suspending bodies, host/adapter call-bearing bodies, accepted task/dialogue/stream values, accepted method values, adapter thunks, and persistence remain unaccepted. | `docs/reviews/requests/2026-07-08-seq-07.7-function-stack-non-helper-callable-allocation.md`; `docs/implementation/function-stack-non-helper-source-function-values-2026-07-09.md`; `docs/implementation/function-stack-source-function-unaccepted-source-call-rejection-2026-07-09.md`; `docs/implementation/function-stack-non-helper-callable-kind-rejection-2026-07-09.md`; `docs/implementation/function-stack-data-last-callable-kind-partial-rejection-2026-07-09.md`; `docs/implementation/function-stack-method-value-rejection-2026-07-09.md`; `docs/implementation/function-stack-current-gap-map-2026-07-09.md` |
| Final closure effect-row model | Current effect composition is broad and useful. The path audit is complete, `no_effect` now has focused closure-invocation, partial-closure-invocation, returned-closure-invocation, and partial-curried-higher-order invocation coverage, captured function aliases preserve rows through returned closures, borrowed captures crossing `await` preserve closed row evidence while reporting the lifetime diagnostic, current LSP diagnostics surface returned-closure and performed-effect traces as related information, typed closed row boundary projection exists, `EffectAnalysisReport` owns current row-substitution resolution for downstream closed reports, Agent artifact verified-effects lowering consumes `ClosedEffectRowReport` directly, LSP callable declaration hover displays rows from the owned raw row report, closure expression hover consumes the same raw row boundary and can render open row labels, `EffectAnalysisReport` exposes `EffectTraceReport` row-origin witnesses for inferred effects, semantic `Function` types now carry known callable rows, closed source function type row syntax is implemented, closed function type rows constrain closure expected types, closure expressions export typed evidence that joins their expression judgment to the synthetic callable used by closed row reports, and sema exposes an owned lookup API for that callable evidence. Open-row inference, closure/higher-order row variables, polymorphic upper-bound syntax beyond the current closed row suffix, and runtime-plan/verifier consumers beyond current artifact/hover/trace-report/function-type/callable-evidence proof are still not finalized. | `docs/reviews/requests/2026-07-08-seq-07.8-function-stack-closure-effect-row-final-contract.md`; `docs/implementation/function-stack-closure-effect-row-audit-2026-07-09.md`; `docs/implementation/function-stack-effect-row-closed-boundary-2026-07-09.md`; `docs/implementation/function-stack-effect-row-report-boundary-2026-07-09.md`; `docs/implementation/function-stack-effect-row-artifact-closed-input-2026-07-09.md`; `docs/implementation/function-stack-effect-row-lsp-hover-2026-07-09.md`; `docs/implementation/function-stack-closure-effect-row-lsp-hover-2026-07-09.md`; `docs/implementation/function-stack-effect-row-raw-lsp-hover-2026-07-09.md`; `docs/implementation/function-stack-effect-trace-report-2026-07-09.md`; `docs/implementation/function-stack-function-type-effect-rows-2026-07-09.md`; `docs/implementation/function-stack-source-function-type-effect-row-syntax-2026-07-09.md`; `docs/implementation/function-stack-closure-expected-effect-row-bound-2026-07-09.md`; `docs/implementation/function-stack-closure-effect-callable-evidence-2026-07-09.md`; `docs/implementation/function-stack-effect-callable-report-api-2026-07-09.md`; `docs/implementation/function-stack-effect-row-partial-closure-timing-2026-07-09.md`; `docs/implementation/function-stack-effect-row-returned-closure-no-effect-2026-07-09.md`; `docs/implementation/function-stack-lsp-performed-effect-trace-2026-07-09.md`; `docs/implementation/function-stack-effect-row-curried-higher-order-timing-2026-07-09.md` |
