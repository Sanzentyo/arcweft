...
- A narrow non-helper source-local top-level `fn` family now materializes as
  ...
  accepts simple-parameter expression bodies with no calls, pipes, `await`,
  `try`, or suspension/effect-capable syntax. Multiple curried `ParamGroup`s
  lower to nested functions, returned simple closure literals recursively lower
  to nested runtime functions, and named missing-input partial calls synthesize
  wrapper functions that preserve declaration argument order.
