...
- A narrow non-helper source-local top-level `fn` family now materializes as
  ...
  declaration argument order, and exact named pure-helper calls preserve helper
  input order. Pure value-position `if`, `if let`, and `match` expressions in
  this accepted family retain `RuntimeExpr::If`, `RuntimeExpr::IfLet`, and
  `RuntimeExpr::Match` shapes; `if let` guards see the pattern locals they
  guard.
