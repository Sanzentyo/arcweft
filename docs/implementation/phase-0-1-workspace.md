...
- Control-transfer statements preserve Rust-like label references for `break 'label expr`, `continue 'label`, and `out 'label expr` so later diagnostics and label-resolution passes can name the intended continuation without treating the statement as raw syntax.
