...
- Top-level `trait` and `impl` declarations are parsed as structured syntax items. Trait members keep associated type information and structured function signatures, including `self` receivers; impl items keep generics, trait target, implementation target, body, source ranges, and HIR declaration preservation.
