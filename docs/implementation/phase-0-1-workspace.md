...
compatibility re-exports; they import through `model`, `lower`, or the
namespaced `syntax` module. The provisional `id_context` namespace was
later deleted with its raw-source ID materializer; current consumers import
through `model`, `lower`, or `syntax`.
...
| `arcweft-lang-hir` split (`model`, `lower`, `lower_*`) | Complete; provisional ID scanner later deleted | Public namespace structure remains split by responsibility; the raw-source `id_context` owner no longer exists. |
...
| `arcweft-lang-syntax` parser family split | Complete for review21 split | Family modules exist for top-level, flow, control-flow, statements, await, dialogue, choice, line-plan, items, hooks, source, proof, recovery, helpers, and headers. `parser.rs` now owns the parse driver/state and top-level lifecycle, while cross-cutting common helpers live in `parser/helpers.rs`. |
...
| syntax AST/parser split | Complete for review21 split | AST family split is complete (`crates/arcweft-lang-syntax/src/ast/{common,ids,items,pattern,flow,dialogue,line_plan,choice,proof,source}.rs`); parser family modules exist (`crates/arcweft-lang-syntax/src/parser/{await_,choice,control_flow,dialogue,flow,headers,helpers,hooks,items,line_plan,proof,recovery,source,statements,top_level}.rs`). `parser.rs` is down to the parse driver/state plus lifecycle plumbing; common helpers moved to `parser/helpers.rs`. |
| HIR/runtime-plan split | Complete | HIR: `crates/arcweft-lang-hir/src/{model,lower,lower_flow,lower_dialogue,lower_choice,lower_ids,lower_context}.rs`; the provisional raw-source `id_context.rs` was later deleted. Runtime-plan: `crates/arcweft-runtime-plan/src/{errors,expr,flow,labels,line_task,pattern,source,stream}.rs` with namespace root in `src/lib.rs`. |
...
- Value-producing `let PAT = if COND { ... } else { ... }` expressions parse into structured expression nodes with block-expression branches. The minimal checker validates the condition as `bool`, scopes branch-local statements, and joins differing branch result types as anonymous sums.
- Value-producing `let PAT = if let BIND = EXPR when GUARD { ... } else { ... }` expressions parse into structured expression nodes. The minimal checker validates guards as `bool`, scopes successful pattern bindings to the then branch, and keeps compatible or anonymous-sum branch joins typeable.
- Value-producing `let PAT = match EXPR { PAT when GUARD => EXPR ... }` expressions parse into structured expression nodes. The minimal checker validates guards as `bool`, walks arm patterns and values for symbol collection, scopes arm-local bindings, and joins differing arm result types as anonymous sums.
...
- `lower_document_to_hir` verifies that parsed edge-case flow syntax belongs to
  the exact supplied source-document revision, converts it to HIR-facing
  structures, and rejects raw flow recovery nodes that still need parser
  coverage. Flow, statement, choice, choice-plan, and line-plan recovery nodes
  preserve source text, grammar family, and source span metadata through typed
  syntax so diagnostics, verifier obligations, and LSP actions do not need to
  re-scan raw source.
