...
| Simple non-helper source-local `fn` values | Ordinary `fn` declarations with simple identifier parameters and expression bodies that contain no host/effect/suspension-capable syntax materialize as `RuntimeExpr::Function` without using the pure-helper table. Multiple curried `ParamGroup`s lower to nested functions. Returned simple closure literals also lower to nested runtime functions when the closure body stays inside the same accepted expression subset. Direct calls to function-typed parameters lower as `RuntimeExpr::Apply` against the local function value. Function-valued `let` aliases and partial calls are tracked inside the accepted body. Named missing-input partial calls synthesize wrapper functions that preserve declaration argument order. | `docs/implementation/function-stack-non-helper-source-function-values-2026-07-09.md`; `checked_runtime_plan_materializes_named_missing_source_function_partial_call`; `checked_runtime_plan_materializes_curried_source_function_value`; `checked_runtime_plan_materializes_source_function_returned_closure`; `checked_runtime_plan_materializes_source_function_callback_param_call`; `checked_runtime_plan_materializes_source_function_callback_partial_let`. |
