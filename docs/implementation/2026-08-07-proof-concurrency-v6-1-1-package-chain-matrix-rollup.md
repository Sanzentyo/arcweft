...
| `FLOW_ADJ_2_1_1_1` | `F06`, `T-ITEM-F-06`, `T-ITEM-T-06`, `T-ITEM-R-06`, and F06 portion of `T-BODY-09` | `EFFECTIVE_WITH_NORMALIZATION` | returned phrase “scope child of match statement scope” | Match owns no container ScopeId. The scrutinee uses the inherited outer scope. Each ordinary arm has a distinct outer-parented `MatchArm` typed-owned by the Match ID; only an authored BlockExpr value nests a Block. A braced Thread arm has one Block as both arm scope and nested Thread-body owner, with no parallel MatchArm. | HIR Match expression/statement lowering, scope-graph freeze, Thread nested-body projection, and runtime once-evaluation/binding-cleanup tests. | `IMPLEMENTED_NOT_RUN` | Exact normalized F06 execution mapping pending. |
