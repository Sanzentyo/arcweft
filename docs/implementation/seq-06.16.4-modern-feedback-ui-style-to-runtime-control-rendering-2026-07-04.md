# Implementation note: seq06.16.4 modern feedback UI style-to-runtime-control rendering

## Audit summary

Current repo inspection showed the missing link is exactly at the product UI/runtime-control boundary:

- `UiStyleResource` already stores rules, part rules, typed values, interaction states, element states, and system colors.
- `UiRuntimeTextControl` and `UiRuntimeActionButton` currently carry values/bounds/actions but no visual style.
...
- Adds data-only `UiRuntimeControlStyle` / `UiRuntimeControlVisualStyle` / border / focus-ring / shadow payloads.
...
- Adds resolver methods on Arcweft-owned types:
  ...
  - `UiStyleResource::runtime_text_control_style`
  - `UiStyleResource::runtime_action_button_style`
  - `UiInputResource::runtime_text_controls_with_style`
  - `UiProgramResource::runtime_action_buttons_with_style`
- Adds `style` fields to runtime text/action controls with serde defaults for backward decode compatibility.
- Adds optional `style` id on `UiActionButtonResource` for direct authored action-button style targeting.
...
- Adds one conversion module that lowers `UiRuntimeControlStyle` into renderer-owned `RenderControlStyle`.
...
- Inline CSS patch text is not re-parsed at runtime; it must be lowered into `UiStyleResource` by the product compiler.
...
| `crates/arcweft-bundle/src/resource_codec/ui.rs` | 770 | 18 | production | false |
| `crates/arcweft-bundle/src/resource_codec/ui/model.rs` | 46781 | 1555 | production | true |
| `crates/arcweft-bundle/src/resource_codec/ui/runtime_control_style.rs` | 29945 | 889 | production | false |
| `crates/arcweft-bundle/tests/runtime_control_style_resolution.rs` | 8241 | 240 | test | false |
| `crates/arcweft-bundle/tests/ui_action_button_resources.rs` | 2165 | 56 | test | false |
