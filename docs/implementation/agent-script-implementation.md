...
- `cargo run -p arcweft-cli -- agent script run samples/agent-script/cli-read-resource-smoke.awfagent --json --trace-out target/codex-agent-script-final/cli-read-resource-smoke.arcwx`
...
- Extend CLI script run and MCP tooling toward deeper graph/RAG fusion across persisted DB and trace context, typed nested resource body projection, and MCP control surfaces beyond the current observe/action/capture/resource/state/signal/log/debug-search/trace/RAG tools.
