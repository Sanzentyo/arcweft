...
- Type Agent references against actions and resource field shapes beyond the current choice/layer/signal/metric plus typed inline image-action project-index coverage and typed `read_resource(...) -> AgentResource`.
...
- Extend CLI script run and MCP tooling toward deeper graph/RAG fusion across persisted DB and trace context, typed resource field projection, and MCP control surfaces beyond the current observe/action/capture/resource/state/signal/log/debug-search/trace/RAG tools.
