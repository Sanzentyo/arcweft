...
- `arcw agent repl <file.arcw|--profile <id>> --input <session.txt> --json` reads scripted cells from a file or stdin, keeps meta commands under the `:` namespace, uses the shared Agent fragment parser to classify non-meta cells, and delegates `:observe` / `:actions` to the native Agent observation/action-target surface. `:query TEXT` reuses the same observation-derived `RagContextPack` builder as Agent MCP RAG query, while `:history` records prior REPL cell outcomes. Non-meta cells are now compiled by wrapping the fragment in a single synthetic Agent controller and running it through the existing `arcweft-compiler` plus `arcweft-agent-runner` deterministic CLI session path; successful cells are recorded as in-session `cell.N` binding artifacts visible through `:bindings`. VM-local variable extraction and SQLite-backed `repl_cells` persistence remain follow-up work rather than being faked by a second evaluator.
...
- `compile_agent_source` remains the parser/HIR-only entry point for lightweight syntax checks; `compile_agent_source_with_project` is the typed project-index entry point; `compile_agent_bundle_with_project` is the typed artifact entry point. Controller VM execution and host-call dispatch are owned by `arcweft-agent-runner`; the CLI native game adapter path currently supports observe, wait polling, capture, resource read, semantic `SelectChoice`, enabled-observation-gated `AdvanceText` including native Game-mode E2E validation, semantic `Invoke`, privacy-filtered standalone trace-backed CLI RAG query, privacy-filtered standalone debug DB lexical, stored-vector, graph-edge, and history search, deterministic REPL observe/actions/query/history/bindings meta commands, compiler-backed deterministic REPL cell execution for complete Agent fragments, and MCP observe/action/capture/resource/state/signal/log/debug-search/trace/RAG tools through the existing native Agent resource/runtime builders. VM-local REPL binding extraction, SQLite-backed REPL cell persistence, graph expansion/fusion beyond direct DB edge search, and MCP control surfaces beyond semantic action dispatch remain follow-up work.
...
- `cargo run -p arcweft-cli --features native-capture -- agent repl samples\rich-text-showcase.arcw --input samples\agent-script\repl-smoke.txt --steps 1 --max-ops 64 --json`
