...
- Extend `.arcwx` beyond CLI run trace writing, validation, capture blob refs, deterministic CLI byte-backed blob storage/validation, read-only logical replay, Sans-I/O MCP resource conversion, stdio MCP trace file loading/resources, and native game-adapter capture refs: connect native capture resource bytes to the same content-addressed blob storage and validation path.
