...
- Extend CLI script run from native observe/wait/capture/resource/semantic `SelectChoice`/`Invoke` plus enabled-observation-gated `AdvanceText` sessions to REPL, broader standalone CLI debug search beyond trace-backed RAG, and MCP control surfaces beyond the current observe/action/capture/resource/state/signal/log/trace/RAG tools.
