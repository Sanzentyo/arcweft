...
- Extend CLI script run and MCP tooling toward deeper graph/RAG fusion across persisted DB and trace context, typed resource body schemas beyond the current JSON/text/base64 string projections, and MCP script-run/control surfaces beyond the current observe/action/wait/capture/resource/state/signal/log/debug-search/trace/RAG tools.
