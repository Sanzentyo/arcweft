...
- Type Agent references against deeper action/resource/body field shapes beyond the current choice/layer/signal/metric plus typed inline image-action project-index coverage, explicit physical pointer action surface, typed `read_resource(...) -> AgentResource` metadata fields, and `attach(CaptureRef|AgentResource)` debug-record coverage.
