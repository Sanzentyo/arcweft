...
- Extend CLI script run and MCP tooling toward deeper graph/RAG fusion across persisted DB and trace context, typed resource body schemas beyond the current JSON/text/base64 string projections, native runtime value/pure-backend script-run arguments, and MCP control surfaces beyond the current observe/action/wait/script-run/capture/resource/state/signal/log/debug-search/trace/RAG tools.
