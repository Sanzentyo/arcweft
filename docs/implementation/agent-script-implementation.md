...
- Extend CLI script run from native observe/wait/capture/resource/semantic `SelectChoice`/`Invoke` plus enabled-observation-gated `AdvanceText` sessions to MCP graph/vector/history debug-search surfaces, deeper graph/RAG fusion across persisted DB and trace context, and MCP control surfaces beyond the current observe/action/capture/resource/state/signal/log/debug-search/trace/RAG tools.
