...
- Type Agent references against deeper action/resource/body field shapes beyond the current choice/layer/signal/metric/flow plus typed inline image-action project-index coverage, `Observation.actions.contains(choice_action(...))`, indexed `ActionTarget` field projection, explicit physical pointer action surface, typed `read_resource(...) -> AgentResource` metadata fields, heterogeneous RAG root references, and `attach(CaptureRef|AgentResource)` debug-record coverage.
