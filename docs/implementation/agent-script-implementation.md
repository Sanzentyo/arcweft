...
- `arcweft-agent-runner` provides the `AgentSession` host boundary, deterministic runtime policy checks, bounded wait polling with stable-frame confirmation, debug event emission, and a RAG service boundary for controller host calls. It can now run Agent controller bytecode through the shared core bytecode VM, dispatch effect-form Agent calls such as `observe(...)`, `checkpoint(...)`, `attach(...)`, `advance_text()`, `choose(...)`, `invoke(...)`, explicit `pointer.click(...)`, `capture(...)`, `read_resource(...)`, and `rag.query(...)`, evaluate structured wait predicates including `Compare`, `Exists`, `All`, `Any`, and `Not` against signal, metric, state-payload, and observation-field probes, and bridge Agent `HostTaskRequest::Custom` suspend/resume tasks back into the VM so expressions such as `let shot = capture(...)` can bind the returned capture record and continue execution. Observation VM payloads now include action target records and signal maps so expressions such as `frame.actions.contains(choice_action(@choice...))` and `frame.signals.get(@signal...)` can execute against host observations. Semantic actions use the `agent.act.semantic` runtime capability, physical pointer actions require the separate `agent.act.physical` runtime capability, and debug records require `debug.record`.
...
- Type Agent references against deeper action/resource/body field shapes beyond the current choice/layer/signal/metric plus typed inline image-action project-index coverage, `Observation.actions.contains(choice_action(...))`, explicit physical pointer action surface, typed `read_resource(...) -> AgentResource` metadata fields, and `attach(CaptureRef|AgentResource)` debug-record coverage.
