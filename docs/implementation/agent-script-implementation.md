...
- Extend CLI script run from native observe/wait/capture/resource/semantic `SelectChoice`/`Invoke` plus enabled-observation-gated `AdvanceText` sessions to compiler-backed transactional REPL DSL cell execution and binding persistence, graph/history standalone CLI debug search beyond current trace-backed RAG and DB/MCP lexical plus DB vector search, and MCP control surfaces beyond the current observe/action/capture/resource/state/signal/log/debug-search/trace/RAG tools.
