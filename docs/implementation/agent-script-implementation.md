...
- Extend CLI script run from native observe/wait/capture/resource/semantic `SelectChoice`/`Invoke` plus enabled-observation-gated `AdvanceText` sessions to compiler-backed transactional REPL cell execution, broader standalone CLI debug search beyond trace-backed RAG, and MCP control surfaces beyond the current observe/action/capture/resource/state/signal/log/trace/RAG tools.
