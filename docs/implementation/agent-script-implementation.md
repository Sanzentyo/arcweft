...
- `arcweft-agent-runner` validates decoded Agent controller bundle project bindings before starting the controller VM. `ProjectBindingMode::Strict` now rejects a runtime `AgentSessionInfo.program_hash` mismatch with a typed `ProjectBindingMismatch` runner error before any host calls or debug events are emitted.
...
- Complete `ProjectBindingMode::Compatible` runtime validation by comparing manifest `required_entities` with the active runtime project entity index. The current runner validates `Strict` program-hash equality before execution, but compatible-mode entity kind/value-type matching still needs a typed runtime project metadata boundary instead of guessing from session strings.
