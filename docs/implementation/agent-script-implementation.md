...
- Extend CLI script run from native observe/wait/capture/resource/semantic `SelectChoice`/`Invoke` plus enabled-observation-gated `AdvanceText` sessions to native interactive `AdvanceText` Game-mode E2E validation, REPL, broader standalone CLI debug search beyond trace-backed RAG, and MCP action/control surfaces beyond the current read-only observe/capture/resource/state/signal/log/trace/RAG tools.
...
- Continue Windows end-to-end validation as interactive `AdvanceText`, REPL, and MCP action/control surfaces are implemented.
