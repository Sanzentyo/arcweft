...
- Extend CLI script run from native observe/wait/capture/resource/semantic `SelectChoice` plus enabled-observation-gated `AdvanceText` sessions to native `Invoke` action dispatch, native interactive `AdvanceText` Game-mode E2E validation, REPL, standalone CLI debug search/RAG commands using the shared JSON/resource shapes, and MCP action/control surfaces beyond the current read-only observe/capture/resource/state/signal/log/trace/RAG tools.
