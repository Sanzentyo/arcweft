...
- Extend CLI script run from native observe/wait/capture/resource/semantic `SelectChoice` plus enabled-observation-gated `AdvanceText` sessions to native `Invoke` action dispatch, native interactive `AdvanceText` Game-mode E2E validation, REPL, debug search, and RAG commands using the shared JSON/resource shapes. Add the matching MCP surfaces.
