...
- Agent intrinsic typing includes `expect(condition, message?)`, `deny(condition, message?)`, `signal(ref) -> Probe<T>`, `metric(ref) -> Probe<T>`, `state(String) -> Probe<AgentValue>`, `observation(String) -> Probe<AgentValue>`, `Probe<T>.eq/ne/gt/ge/lt/le(T) -> Predicate`, `exists(Probe<T>)`, `all(...)`, `any(...)`, `not(...)`, `wait(predicate, timeout=...) -> Observation` with timeout required and literal `stable_frames`/`poll_frames` values required to be at least 1, `advance_text() -> ActionResult`, `choose(Ref<ChoiceOption>) -> ActionResult`, `choice_action(Ref<ChoiceOption>) -> ActionTarget`, `Observation.actions -> Vec<ActionTarget>`, `Observation.signals -> Map<AgentValue, AgentValue>`, `contains(...)` for typed sequences, `get(...)` for typed maps, `invoke(target, action, args?) -> ActionResult` against project-index Agent action signatures with named payload parameter validation, including typed inline `image(... target=..., action/actions=...)` presentation actions, `viewport_point(x, y) -> ViewportPoint`, explicit `pointer.click(point, button=.primary) -> ActionResult` requiring `agent.act.physical`, `capture(viewport|layer|object, ...) -> CaptureRef`, `read_resource(String|uri=String) -> AgentResource` with typed `uri`/`kind`/`mime_type`/`hash` field projection and nested `body.kind`/`body.json`/`body.text`/`body.base64`/`body.encoding` string projections plus typed `body.value` payload projection, `attach(CaptureRef|AgentResource)`, `checkpoint(String)`, `note(DisplayText)`, and `rag.query(String, roots=..., graph_depth=..., limit=...) -> RagContextPack`.
