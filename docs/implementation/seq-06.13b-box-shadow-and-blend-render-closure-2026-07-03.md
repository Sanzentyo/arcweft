...
| --- | --- |
| Decide box-shadow route | Arcweft-owned compositor group effect with pure planning in `ui_box_shadow.rs`. |
| Outer shadow | `UiBoxShadowKind::Outer`, `UiBoxShadowPassPlan`, and `PASS_BOX_SHADOW`. |
| Inset shadow | `UiBoxShadowPlanError::InsetUnsupported`. |
| Spread / negative spread | `UiBoxShadowPass::from_outer_shadow` expands or shrinks the caster rect. |
...
| Transparent colors | `UiBoxShadow::is_identity` removes no-op shadows. |
| Opacity/filter/clip/mask/blend interaction | `UiCompositor::render_group` draws shadows before children, then runs the existing group pass chain. |
| HSL/luminosity blend modes | Existing `UiBlendShaderMode` and WGSL HSL branch are preserved by tests. |
...
`UiCompositingEffects::box_shadows`. This cut intentionally stops at the
...
`compositing_effects_from_takumi` emits an empty `UiBoxShadowList` until that
...
enforced by keeping the implementation entirely in the `UiScene` /
`UiCompositor` / WGSL path.

## Validation Commands

```bash
cargo fmt --all -- --check
cargo test -p arcweft-render-wgpu ui_box_shadow --all-features -- --nocapture
...
`4 error(s), 125 warning(s)`. The new `ui_box_shadow.rs` is 222 physical LOC,
`ui_scene/compositing.rs` is 746 physical LOC, and `ui_compositor.rs` is 1080
...
- CSS/Takumi lowering into `UiBoxShadowList` is still follow-up work.
