...
| Source surface can author static/animated image ids and bounded image objects | `asset @asset...` entity declarations in `samples/image-animation.arcw`; `bg(...)`; bounded `image(asset = ..., ...)`; sema tests for declared image assets and image object calls | Partially covered |
| Source-level declared image object syntax beyond bounded `image(...)` calls | Current surface still uses `bg(...)` and bounded `image(...)` calls. A future declaration form must lower into the same `ImagePresentationObject` model if the language chooses to add it. | Open |
...
| Samples cover static and animated formats plus bounded objects | `samples/image-animation.arcw` declares PNG, JPEG, static WebP, GIF, animated WebP, sprite overlay, clipped object, alignment object, opacity, transform, action, params, proxy, and pinned playback | Covered |
...
- Decide whether Arcweft needs a top-level declared image object surface beyond
  bounded `image(asset = ..., ...)` calls. If added, it must lower directly into
  `ImagePresentationObject` and share the same capture, hit-test, bundle, and
  Agent metadata path. Do not add a compatibility syntax branch or a renderer
  shim.
