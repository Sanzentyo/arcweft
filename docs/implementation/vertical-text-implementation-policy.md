...
- CLI regression coverage includes an `imq` native vertical capture parity check
  ...
  as well: Sans I/O layout keeps `$1234`, `¢1234`, `50%`, `$五`, and `五%`
  unbroken across text-combine chunking and ideographic numeral clusters, while
  native Agent geometry/raw mask/object-id crops verify that the `$` and `¢`
  prefix objects and `%` suffix object are read back from rendered GlyphArea
  pixels for Arabic and ideographic numerals.
