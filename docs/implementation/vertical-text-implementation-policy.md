...
- CLI regression coverage includes an `imq` native vertical capture parity check
  ...
  iteration-mark, and strict middle-dot `vertical_lr` paths through mask and
  object-id attachments, tying the mirrored observed bboxes and stable object
  colors back to rendered GlyphArea pixels.
