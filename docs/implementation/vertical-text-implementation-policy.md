...
- Typewriter reveal is applied after layout as glyph color alpha in the GlyphArea
  ...
  resolved presentation into their absolute GlyphArea instances, so ruby object
  mask and object-id readback can hide and reveal the annotation at capture time
  without changing the observed base/annotation bboxes.
