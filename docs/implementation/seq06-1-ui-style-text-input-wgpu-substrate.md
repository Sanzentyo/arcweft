# seq06.1 UI style, text input, and direct wgpu substrate
...
- Public display text remains a single `Text` view; rich, localized, and
  display-frame sources stay as retained `UiTextSource` variants.
...
- Component internals are targetable through typed exported parts (`UiPartId` and
...
- Product UI compact resource codecs remain seq02.4.1.
...
```bash
cargo fmt --all -- --check
cargo check -p arcweft-presentation -p arcweft-ui -p arcweft-lang-syntax -p arcweft-render-wgpu
cargo test -p arcweft-presentation -p arcweft-ui -p arcweft-lang-syntax -p arcweft-render-wgpu
...
cargo fmt --all -- --check
cargo check -p arcweft-presentation -p arcweft-ui -p arcweft-lang-syntax -p arcweft-render-wgpu
cargo test -p arcweft-presentation -p arcweft-ui -p arcweft-lang-syntax -p arcweft-render-wgpu --quiet
cargo clippy -p arcweft-presentation -p arcweft-ui -p arcweft-lang-syntax -p arcweft-render-wgpu --all-targets --all-features -- -D warnings
...
- `arcweft-ui::style`, `style_authoring`, `program`, and `text_field` own
