...
| Transparent shadow | Implemented identity | `UiBoxShadowList::new` + test | Empty list / no pass. |
...
| Inset shadow | Structured diagnostic | `UiBoxShadowPlanError::InsetUnsupported` test | Not visually rendered in seq06.13d. |
...
| `filter: drop-shadow(...)` | Preserved separate path | test | Lowers to `UiFilter::DropShadow`. |
