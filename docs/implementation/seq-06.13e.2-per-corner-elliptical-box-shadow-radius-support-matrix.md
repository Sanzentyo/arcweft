...
| Takumi computed solid background radii | Implemented | `ViewSurfaceBackground::Solid { radii }` | Computed `border-*-radius` values lower into renderer-owned `ViewCornerRadii` and reach retained `ViewRoundedRect` fills through the same surface primitive builder used by runtime surfaces. Gradient/image rounded clipping still needs a separate clip/mask primitive expansion. |
