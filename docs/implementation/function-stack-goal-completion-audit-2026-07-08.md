...
| Partial-application desugaring | Helper-backed prefix partials, named/fixed missing-input partials, local aliases, runtime apply, and the first simple non-helper source-local `fn` materialization are implemented, including curried groups, returned simple closure literals, and direct calls to function-typed parameters in that accepted family. Helper-less signature partials still fail as unsupported callable family `signature_partial_without_helper` when no pure helper or accepted source-function candidate exists; spread partials remain request/design work. | Partially implemented; spread split to 07.2.1 and broader non-helper expansion remains in 07.7 |
...
3. General non-helper/effectful/suspending callable allocation:
   ...
   closure literals plus direct calls to function-typed parameters, but
   host/top-level call-bearing, effectful, suspending callable values,
