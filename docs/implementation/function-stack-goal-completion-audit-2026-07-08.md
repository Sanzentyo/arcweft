...
| Partial-application desugaring | Helper-backed prefix partials, named/fixed missing-input partials, local aliases, and runtime apply are implemented. Helper-less signature partials now fail as unsupported callable family `signature_partial_without_helper`; spread partials remain request/design work. | Partially implemented; spread split to 07.2.1 and non-helper expansion split to 07.7 |
