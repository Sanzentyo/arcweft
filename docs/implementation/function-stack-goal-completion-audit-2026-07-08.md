...
| Partial-application desugaring | Helper-backed prefix partials, named/fixed missing-input partials, local aliases, runtime apply, and the first simple non-helper source-local `fn` materialization are implemented, including curried groups, returned simple closure literals, direct calls to function-typed parameters, local callback aliases/partials, destructuring closure literals in local function-valued bindings, exact pure-helper calls in that accepted family, inline fixed-length literal spread in function-value calls, and inline fixed-length literal spread in direct fixed-parameter signature exact/partial calls. Helper-less signature partials still fail as unsupported callable family `signature_partial_without_helper` when no pure helper or accepted source-function candidate exists; data-last spread fallback and variable-length spread remain request/design work. | Partially implemented; spread split to 07.2.1 and broader non-helper expansion remains in 07.7 |
