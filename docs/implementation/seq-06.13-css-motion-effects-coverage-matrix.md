...
| Per-corner / elliptical `box-shadow` radii | Implemented seq06.13e.2 | `UiBoxShadowRadii`, Takumi corner lowering, planner diagnostics, WGSL per-corner coverage | Scalar constructors remain convenience APIs; exact PNG promotion remains pinned-environment only. |
