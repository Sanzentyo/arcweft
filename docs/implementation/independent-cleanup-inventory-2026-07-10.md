...
| 10 | Duplicate dialogue content model | Resolved in a later deletion cut: the lossy `arcweft-dialogue` facade was removed. | Structural syntax/HIR/sema content and the shared runtime RichText model are the only authorities; no conversion or compatibility reader remains. |
