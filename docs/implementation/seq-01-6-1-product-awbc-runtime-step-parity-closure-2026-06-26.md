...
## Integration update 2026-06-26, fourteenth cut

Current working change before commit: `pomrlyrx` over parent `wmmwqvwm`.

This cut closes the non-control line-effect differential surface:

- added a table fixture for structured/AWBC product parity across
  `RegisterHandle`, `DropHandle`, duration `Wait`, `Call`, `Log`,
  `SignalWrite`, `MetricWrite`, `EmitEvent`, `Out`, `Ensure`, `Assert`,
  `Close`, `Select`, `Break`, and `Continue`;
- asserts the structured runtime and AWBC product facade emit the same
  `RuntimeStepOutput.effects.line` payload at the same step boundary;
- keeps control-changing effects (`Return`, `Goto`, `Panic`, `Fail`, `Bail`)
  outside this row because they change runtime control flow, not only the
  effect list;
- keeps `Audio` outside this row because product-step currently reports a
  typed unsupported diagnostic until an audio command payload contract is
  designed.
...
cargo test -p arcweft-runtime-plan awbc_product_parity_line_effect_kind_table_non_control -- --nocapture
  passed: 1 passed, 0 failed
...
cargo test -p arcweft-runtime-plan awbc -- --nocapture
  passed: 1 awbc_lower unit test and 35 awbc_product_parity tests passed,
  0 failed
...
cargo +nightly -Zscript tools/structure-audit.rs --root .
  passed with no error-level violations: files scanned 1514, Rust files 833,
  Rust physical LOC 410412, package manifests 89, warnings 102.

git diff --check
  passed
```

Structural measurements for files touched in the fourteenth cut:

| Path | Bytes | Physical LOC | Kind / responsibility |
|---|---:|---:|---|
| `crates/arcweft-runtime-plan/tests/awbc_product_parity.rs` | 50819 | 1519 | integration differential parity tests; below the 2500 LOC integration-test warning threshold |
...
- source handler item/progress/error pattern dispatch, source-yield queue
  ...
  behavior, and non-control line-effect payload parity are now differentially
  covered.
- multi-stream yield and self-close routing is covered for ordinary static
  stream targets, and stream-to-source static close is differentially covered.
  Dynamic close-target expressions remain unsupported by the compact AWBC
...
## Integration update 2026-06-26, sixteenth cut

Current working change before commit: `zttyxtul` over parent `tpqomzyo`.

This cut is a structural split before the product-step adapter crosses the
2,500 LOC error threshold:

- moved the product control-effect adapter into
  `crates/arcweft-core/src/awbc/product_step/control.rs`;
- kept the public product-step surface unchanged and left the call sites as
  `self.apply_control_effects(...)`;
- kept the moved responsibility narrow: it owns only the newly emitted
  control-effect scan, `Return`/`Goto`/failure application, and the product
  public-id lookup needed by `Goto`;
- reduced `product_step.rs` from 2,485 LOC to 2,396 LOC while adding a 98 LOC
...
cargo +nightly -Zscript tools/structure-audit.rs --root .
  ...
  Rust physical LOC 410613, package manifests 89, warnings 102.
...
Structural measurements for files touched in the sixteenth cut:

| Path | Bytes | Physical LOC | Kind / responsibility |
|---|---:|---:|---|
| `crates/arcweft-core/src/awbc/product_step.rs` | 90802 | 2396 | production product AWBC RuntimeStepResult adapter; warning-level size hotspot, below the 2500 LOC error threshold |
| `crates/arcweft-core/src/awbc/product_step/control.rs` | 3500 | 98 | production product AWBC control-effect adapter |
...
## Integration update 2026-06-26, eighteenth cut

Current working change before commit: `wlyuwkpv` over parent `luxstntu`.

This cut closes the final/output facade equality coverage in the differential
harness:

- captures the structured and AWBC product `FlowFiber` facade snapshots for
  every parity step;
- compares facade environment, observation state, source states, stream states,
  and normalized facade status at `Done`, `Failed`, and `Output` step
  boundaries;
- keeps internal cursor, line cursor, pending-op, and control-stack comparison
  outside this assertion because compact AWBC function/block execution and
  structured `FlowOp` execution intentionally expose different internal cursor
  shapes;
- skips OneOp and budget-exhausted intermediate states for this final-facade
  assertion. A probe that compared every step exposed expected intermediate
  differences: structured OneOp already has the partial structured env while
  product AWBC has not necessarily projected the same register facade, and
  budget-loop structured state may carry a loop scope that compact AWBC does
  not represent as a structured `RuntimeEnv` scope.
...
cargo +nightly -Zscript tools/structure-audit.rs --root .
  passed with no error-level violations: files scanned 1515, Rust files 834,
  Rust physical LOC 410748, package manifests 89, warnings 102.
...
Structural measurements for files touched in the eighteenth cut:

| Path | Bytes | Physical LOC | Kind / responsibility |
|---|---:|---:|---|
| `crates/arcweft-runtime-plan/tests/awbc_product_parity.rs` | 54431 | 1646 | integration differential parity tests; below the 2500 LOC integration-test warning threshold |
...
- source handler item/progress/error pattern dispatch, source-yield queue
  ...
  out-of-order progress/ready behavior, direct suspend/result host-call
  behavior, non-control line-effect payload parity, control-changing
  line-effect parity, typed trap projection, and final/output facade state
  equality are now covered.
- multi-stream yield and self-close routing is covered for ordinary static
  stream targets, and stream-to-source static close is differentially covered.
  Dynamic close-target expressions remain unsupported by the compact AWBC
  instruction shape and currently fail lowering with a diagnostic.
