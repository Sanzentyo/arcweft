...
- `arcweft-lang-jit-cranelift` now owns the first native Cranelift adapter. It
  ...
  The parameterized `i64` and `i32` paths now separate module codegen from JIT
  execution: `define_i64_with_inputs` and `define_i32_with_inputs` define entry,
  row-batch, and row-batch-sum functions into a generic Cranelift `Module`,
  while the corresponding `compile_*_with_inputs` functions remain native JIT
  wrappers that finalize the `JITModule` and install function pointers.
