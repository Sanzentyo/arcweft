...
- Closed source function type rows used as closure expected types now become
  the synthetic closure callable's upper bound. A matching body effect is
  accepted, while `effects { }` rejects a body that performs `fs.read`.
...
- LSP callable and closure effect-row hover now reads the owned raw row report,
  so future open-row evidence can display without forcing closed projection.
