# Modern Feedback View Translucent Style And Depth

## Scope

This slice extends the seq06.16.4 runtime-control style bridge with authored
control depth and updates `samples/modern-feedback-view` to exercise the path with
a bundled background image, translucent controls, rounded corners, and
`box-shadow`.

The background asset at
`samples/modern-feedback-view/src/.arcweft/asset/bg/glass_lights.png` is a
...
- `ViewRuntimeControlVisualStyle` now carries `depth_milli`.
...
- `samples/modern-feedback-view` now renders `image.modern.glass_bg` behind the
...
did not render as retained View compositor nodes and therefore had no typed
...
- `cargo run -p arcweft-cli -- bundle samples/modern-feedback-view/src/main.arcw --output target/arcweft/modern-feedback-view-translucent.awfb`
- `cargo run -p arcweft-cli --features native-capture -- agent observe samples/modern-feedback-view/src/main.arcw --json --image png --capture color --content-policy-mode local-dev --out target/modern-feedback-view/transparent-depth-observe.png --mode drain --steps 4 --max-ops 64`
- `cargo clippy --workspace --all-targets --all-features`
- `cargo +nightly -Zscript tools/structure-audit.rs --root .`

The native observe report produced `diagnostics_count=0` and wrote
`target/modern-feedback-view/transparent-depth-observe.png`.
