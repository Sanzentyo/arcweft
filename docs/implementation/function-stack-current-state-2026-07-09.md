...
- Current closure-effect composition for the broad implemented closure,
  ...
  directly to closed-row consumers. `EffectAnalysisReport` also owns the raw
  `EffectRowReport`; LSP callable and closure effect-row hovers consume that
  raw boundary and can render open-row labels through `EffectRow::display_label`.
  Agent verified-effect artifact lowering
  now consumes `ClosedEffectRowReport` as its direct input instead of accepting
  the full effect-analysis report.
  ...
  closed registered rows, function-value partials preserve those rows, source
  function type syntax accepts closed row suffixes such as
  `A -> B effects { fs.read }`, and source labels display closed non-empty
  rows. Closed rows on function type ascriptions used as closure expected
  types now become the synthetic closure callable's upper bound, so an empty
  expected row rejects body effects instead of leaving the closure unbounded.
  Closure expressions also export typed lowering evidence that joins their
  expression judgment to the
  synthetic callable used by closed row reports. Sema now exposes an owned
  lookup API for that callable evidence, and LSP hover consumes that report API
  on closure headers.
