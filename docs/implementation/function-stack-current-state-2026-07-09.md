...
- A first non-helper source-local `fn` runtime-function subset:
  ...
  destructuring closure literals in those local aliases, and exact calls to
  already-lowered pure helpers.
...
| Broad non-helper callable allocation | The first source-local `fn` subset is implemented. Effectful/suspending bodies, host/top-level non-helper call-bearing bodies, task/dialogue/stream functions, trait/impl methods, adapter thunks, and persisted callable values remain outside the accepted contract. | `docs/reviews/requests/2026-07-08-seq-07.7-function-stack-non-helper-callable-allocation.md` |
