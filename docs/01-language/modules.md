...
Rust exports have no Arcweft source declaration. Rust crates opt in through
deterministic `arcweft-rust-abi` metadata, usually generated by proc macros and
written by a crate `build.rs` into the project. The schema-1 launch manifest
admits that metadata, fixes its expected package/module/family/ABI identity,
and mounts its public exports into the selected profile's module namespace.
...
The generated metadata is the signature authority. Source cannot redeclare,
rename, or partially shadow its exported type and callable inventory.
