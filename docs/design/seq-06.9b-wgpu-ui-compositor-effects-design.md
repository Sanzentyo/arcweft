...
scene contract. The upstream contract owns `ViewPaintNode`, `ViewCompositingGroup`,
`ViewCompositingEffects`, `ViewFilterList`, `ViewMask`, `ViewClipPath`, `ViewBlendMode`,
and `ViewIsolation`; seq06.9b consumes those types and does not redefine them.

The renderer work is split into responsibility modules:

- `view_effects.rs`: pure filter pass planning, color matrices, blur/drop-shadow
  bounds, and texture extent bucketing.
- `view_blend.rs`: shader-supported `mix-blend-mode` classification.
- `view_mask.rs`: mask alpha/luminance pass planning and resource requirements.
- `view_clip_path.rs`: supported clip geometry planning and explicit unsupported
  path diagnostics.
- `view_compositor.rs`: wgpu offscreen target pool, root/group render orchestration,
  backdrop copy path, shader pass execution, and direct primitive callback seam.
- `view_shaders/compositor.wgsl`: full-screen triangle shader for color-matrix,
...
primitive ranges are drawn through `ViewDirectPrimitiveRenderer`; compositing
groups allocate a group target from `ViewRenderTargetPool`, render their children
...
sampling. External mask textures are supplied through `ViewMaskTextureProvider`, so
...
remains an explicit `ViewClipPathPlanError::PathUnsupported` until a vector path
...
`ViewTextureExtent` clamps to a deterministic maximum and buckets intermediate
...
`SharedRenderer::create_view_compositor()` constructs a compositor using the
